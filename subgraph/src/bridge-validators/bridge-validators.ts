...
export function _handlerValidatorAdded(validatorAddress: Bytes, txHash: string): void {
...
export function _handlerValidatorRemoved(validatorAddress: Bytes, txHash: string): void {
  const id = validatorAddress.toHexString()
  let validator = Validator.load(id)

  if (!validator) {
    log.error(`_handlerValidatorRemoved: There was not possible to remove validator with ID: {} NOT FOUND - hash: {}`, [id, txHash])
    return
  }
