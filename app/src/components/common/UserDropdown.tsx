...
import { Dropdown } from '@/src/components/layout/Helpers'
...
const StatusMain = styled(Status)`
  bottom: -1px;
  left: -1px;
  position: absolute;
  z-index: 5;
    ...
    <>
      <Wrapper
        dropdownButton={
          <UserButton>
            <User />
            {!isWalletNetworkSupported && <StatusMain />}
          </UserButton>
        }
        ...
        items={[
          ...
          <ItemDirectionColumn key="1" onClick={() => setShowNetworkModal(true)}>
            {!isWalletNetworkSupported && (
              <ItemTitle>
                <span>Invalid Network</span> <Status />
              </ItemTitle>
            )}
            <TextRow>
